...
Ollama is an _optional_ but powerful dependency that allows PiecesOS to run Large Language Models (LLMs) directly on your device instead of relying on cloud-based AI processing. 
...
Using Ollama for local generative AI is _a separate process_ from how PiecesOS uses local, in-house LoRA models for processing data with the LTM-2 Engine.
